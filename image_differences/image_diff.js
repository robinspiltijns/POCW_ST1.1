...
const image_xor = require('./image_xor');
 ...
 *                                          data.
 *
 * @return  {Promise<Sharp>[]}              Returns array of promises for
...
convert.doImgDiff =  async function (imgs, demand_same_size=false) {
    ...
    const imgs_metas_data_promises = imgs.map(img => {
        ...
        return sharp_img
            .metadata()
            .then(meta => {
                return {meta: meta, sharp_data: sharp_img}
            })
    });
    ...
    const imgs_metas = imgs_metas_data.map(({meta, sharp_data}) => {
        return meta
    });
    ...
    const extend = 400; // The number of pixels we want in the largest dimension.
    ...
    const imgs_data = imgs_metas_data.map(({meta, sharp_data}) => {
        return sharp_data
    });
    ...
    for (let i = 0; i < imgs_buffs.length - 1; ++i) {
        ...
        console.log(new_size.width, new_size.height);
        let val = image_xor.image_xor(imgs_buffs[0], imgs_buffs[i + 1], imgs_buffs[i+1]);
        ...
        to_file_promises.push(sharp(imgs_buffs[i+1], output_meta).toFile(`diff-${i + 1}.png`))
    }
